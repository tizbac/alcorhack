...
set_scsi_pt_flags(struct sg_pt_base * vp, int flags)
{
    ...
    ptp->io_hdr.flags |= SG_FLAG_LUN_INHIBIT | SG_FLAG_DIRECT_IO;
    if (SCSI_PT_FLAGS_QUEUE_AT_HEAD & flags) {  /* favour AT_HEAD */
        ptp->io_hdr.flags |= SG_FLAG_Q_AT_HEAD | SG_FLAG_LUN_INHIBIT ;
        ptp->io_hdr.flags &= ~SG_FLAG_Q_AT_TAIL;
    } else if (SCSI_PT_FLAGS_QUEUE_AT_TAIL & flags) {
        ptp->io_hdr.flags |= SG_FLAG_Q_AT_TAIL | SG_FLAG_LUN_INHIBIT ;
...
do_scsi_pt(struct sg_pt_base * vp, int fd, int time_secs, int verbose)
{
    ...
    ptp->io_hdr.timeout = ((time_secs > 0) ? (time_secs * 1000) :
                                             DEF_TIMEOUT);
    ptp->io_hdr.flags |= SG_FLAG_LUN_INHIBIT;
    printf("%02x\n",ptp->io_hdr.cmdp[1]);
...
set_scsi_pt_flags(struct sg_pt_base * vp, int flags)
{
    ...
    /* default action of block layer SG_IO ioctl is QUEUE_AT_TAIL */
    if (SCSI_PT_FLAGS_QUEUE_AT_HEAD & flags) {  /* favour AT_HEAD */
        ptp->io_hdr.flags |= BSG_FLAG_Q_AT_HEAD | SG_FLAG_LUN_INHIBIT;;
        ptp->io_hdr.flags &= ~BSG_FLAG_Q_AT_TAIL;
    } else if (SCSI_PT_FLAGS_QUEUE_AT_TAIL & flags) {
        ptp->io_hdr.flags |= BSG_FLAG_Q_AT_TAIL | SG_FLAG_LUN_INHIBIT;;
              ...
              int verbose)
{
    ...
    if (BSG_FLAG_Q_AT_HEAD & ptp->io_hdr.flags)
        v3_hdr.flags |= SG_FLAG_Q_AT_HEAD | SG_FLAG_LUN_INHIBIT | SG_FLAG_DIRECT_IO;     /* favour AT_HEAD */
    else if (BSG_FLAG_Q_AT_TAIL & ptp->io_hdr.flags)
        v3_hdr.flags |= SG_FLAG_Q_AT_TAIL | SG_FLAG_LUN_INHIBIT | SG_FLAG_DIRECT_IO;
